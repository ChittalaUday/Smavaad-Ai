...
export const ChatProvider = ({ children }) => {
  ...
  // send message
  const sendChatMessage = async (contentOverride = null) => {
    if (!socket || !currentSelectedChat.current?._id) return;
    const msgContent = contentOverride || message;
  ...
  const editChatMessage = async (messageId, content) => {
    await requestHandler(
      async () => await updateMessage(messageId, content),
      null,
      (res) => {
        setMessages((prevMsgs) =>
          prevMsgs.map((msg) =>
  ...
  const onMessageUpdated = (updatedMessage) => {
    if (currentSelectedChat.current?._id === updatedMessage.chat) {
      setMessages((prevMsgs) =>
        prevMsgs.map((msg) =>
